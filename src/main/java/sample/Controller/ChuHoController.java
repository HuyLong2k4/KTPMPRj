...
public class ChuHoController {
    ...
    public void loadResidents() {
        residentList.clear();
        try (Connection conn = DatabaseConnection.getConnection()) {
            // Chỉ lấy những cư dân là chủ hộ (có resident_id trùng với owned_by trong bảng household)
            String sql = "SELECT r.* FROM resident r JOIN household h ON r.resident_id = h.owned_by";
            PreparedStatement ps = conn.prepareStatement(sql);
